...
Config: engine/config/model-config.local.json
"""
...
# engine/migrations/vector_migration.sql). These are called right after a
...
def write_embedding(
    ...
    ``db`` is a ``engine.core.db.DB``; ``pk_value`` is the source row UUID.
    """
