...
当任务涉及中国 GEO、AI 搜索、品牌营销、市场规模、竞品、用户痛点、融资或长期趋势时，
必须在开始检索前完整读取并执行
[references/geo-research-policy.md](references/geo-research-policy.md)。该规范要求：

1. 按权威行业报告、真实用户与媒体调查、竞品与最新资讯、券商与长期趋势四层检索；
2. 显式搜索当前自然年，证据不足时才回退至此前 24 个月并标注；
3. 对清单中每个适用域名记录已搜索、采用、无相关结果、登录/付费墙、阻断或不适用；
4. 每条事实、数字、比较、用户观点和产品声明后附可点击的原始网页 URL；
5. 报告末尾输出搜索覆盖清单和完整信源清单，并说明每项综合判断基于哪些网址；
6. 不得用搜索摘要、厂商软文或匿名案例代替已打开并核验的原始证据。
