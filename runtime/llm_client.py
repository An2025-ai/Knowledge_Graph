...
Adapted from geo-research/src/llm_client.py. Config is read from
runtime/config/llm-config.local.json by default; API key comes from the
config `api_key` field or the BRAND_ATLAS_LLM_API_KEY env var.
"""
...
DEFAULT_CONFIG_PATH = Path(__file__).resolve().parent / "config" / "llm-config.local.json"
...
class LLMConfig:
    ...
    _note: str | None = field(default=None, repr=False)

    @classmethod
    def from_file(cls, path: Path = DEFAULT_CONFIG_PATH) -> "LLMConfig":
        if not path.exists():
            raise FileNotFoundError(
                f"LLM config not found: {path}. Copy llm-config.example.json to "
                "llm-config.local.json and fill in base_url/model/api_key."
            )
...
class LLMClient:
    ...
    def chat(self, messages: list[dict[str, str]], *, max_tokens: int | None = None) -> str:
        if not self.config.base_url or not self.config.model:
            raise RuntimeError(
                "LLM not configured: base_url/model are empty in llm-config.local.json. "
                "Fill them in, then set the API key via BRAND_ATLAS_LLM_API_KEY."
            )
        body = {
        ...
        headers = {"Content-Type": "application/json", "User-Agent": "BrandAtlasKG/0.1"}
        if key := self.api_key():
