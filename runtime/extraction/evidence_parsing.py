...
from runtime.core.knowledge_service import upsert_evidence_span, upsert_evidence_unit
...
def write_spans(db, document_uuid, span_rows: list[dict]) -> int:
    for sp in span_rows:
        sp = dict(sp)
        sp["document_uuid"] = document_uuid
        upsert_evidence_span(db, sp)
    return len(span_rows)


def write_units(db, document_uuid, unit_rows: list[dict]) -> int:
    for un in unit_rows:
        un = dict(un)
        un["document_uuid"] = document_uuid
        upsert_evidence_unit(db, un)
