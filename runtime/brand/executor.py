...
Each pipeline is a ``run(db, args) -> dict`` in ``runtime.brand.pipelines.<name>``.
...
def load_pipeline(name: str):
    """Import ``runtime.brand.pipelines.<name>`` and return its ``run`` callable."""
    try:
        module = importlib.import_module(f"runtime.brand.pipelines.{name}")
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="python -m runtime.brand.executor",
