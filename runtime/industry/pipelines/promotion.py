...
复用 ``runtime.promotion.promotion_service.evaluate_instance / promote / queue_review``。
"""
...
from runtime.core.knowledge_service import pending_gate_candidates
...
def run(db: DB, args) -> dict[str, Any]:
    ...
    for k in pending:
        ...
        disposition = results.get("disposition", "review")
        if dry_run:
