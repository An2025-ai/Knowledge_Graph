...
Usage:
    python -m runtime.metrics [--db-check] [--json]
"""
...
def compute_metrics(db: DB) -> dict[str, Any]:
    ...
    chunks = db.query("SELECT count(*) AS c FROM document_chunk")[0]["c"]
    m["documents"] = docs
    m["chunks"] = chunks

    # --- Extraction / candidates ---
    cand = db.query("SELECT count(*) AS c FROM extraction_candidate")[0]["c"]
    cand_by_type = db.query(
        "SELECT candidate_type, count(*) AS c FROM extraction_candidate GROUP BY candidate_type"
    )
...
def main() -> int:
    ...
    from runtime.db import check_connection
    ...
    if args.json:
        ...
        print(f"  documents: {m['documents']}, chunks: {m['chunks']}")
        print(f"  extraction candidates: {m['extraction_candidates']} "
        ...
        print(f"  entity embeddings: {m['entity_embeddings']}, pending review: {m['pending_review_items']}")
    return 0
