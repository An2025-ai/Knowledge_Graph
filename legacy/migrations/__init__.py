...
Usage:
    python -m legacy.migrations migrate            # run all (L1 → L2/L3)
    python -m legacy.migrations migrate --only l2_l3  # run only one layer
"""
...
# 统一 legacy database 口径：所有 L2/L3 + 候选向量表收进同一个
# legacy/database/l2_l3_schema.sql。
# 旧的 l2_migration.sql / brand_l3_migration.sql / vector_migration.sql 已删除并并入。
MIGRATIONS = {
    "l1": ROOT / "legacy" / "database" / "schema.sql",
    "l2_l3": ROOT / "legacy" / "database" / "l2_l3_schema.sql",
}
...
def main() -> int:
    ...
    from legacy.core import db as dbmod
