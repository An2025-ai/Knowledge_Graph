...
Usage:
    python -m legacy.brand.executor --pipeline candidate_extraction --file <doc.md> --brand <brand_id> [--dry-run]
    python -m legacy.brand.executor --all --file <doc.md> --brand <brand_id> [--dry-run]

Each pipeline is a ``run(db, args) -> dict`` in ``legacy.brand.pipelines.<name>``.
...
def load_pipeline(name: str):
    """Import ``legacy.brand.pipelines.<name>`` and return its ``run`` callable."""
    try:
        module = importlib.import_module(f"legacy.brand.pipelines.{name}")
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="python -m legacy.brand.executor",
