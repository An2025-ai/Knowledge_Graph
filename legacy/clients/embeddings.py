...
Config: legacy/config/model-config.local.json
"""
...
# legacy/migrations/vector_migration.sql). These are called right after a
...
# Valid vector tables -> primary-key column name.
# 共享新证据/候选/门禁链路 embedding 表（§9）：
#   candidate_embedding / gate_entity_embedding / gate_knowledge_embedding
# 以及保留的 active graph 支撑表 entity_embedding（entity 复用所需）。
_EMBEDDING_TABLES = {
    "candidate_embedding": "candidate_id",
    "gate_entity_embedding": "entity_id",
    "gate_knowledge_embedding": "knowledge_id",
...
def write_embedding(
    ...
    ``db`` is a ``legacy.core.db.DB``; ``pk_value`` is the source row UUID.
    """
