...
```
 ...
 └──────┬──────────────────────────────────────────────────────────┬───────────┘
        │                                                          │
        ▼                                                          ▼
   evidence/候选表（database/l2_l3_schema.sql）              active graph（主图）
                                                                  │
                                                                  ▼
                                                        Neo4j 投影（可重建）
```
...
PostgreSQL 是单一事实源，定义集中在 `database/`：
...
- **唯一依赖单源**：根 `requirements.txt`，可视化脚本与投影均只读它。

## 8. 关键约束

- PostgreSQL 是唯一事实源；Neo4j 随时可 `--full` 重建。
- 新表只能进 `database/l2_l3_schema.sql`；新依赖只能进根 `requirements.txt`。
