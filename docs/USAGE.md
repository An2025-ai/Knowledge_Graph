...
```bash
python scripts/fix_html_stale.py
python scripts/fix_html_cdn.py
```
...
| 脚本 | 作用 |
|------|------|
| `scripts/render_graph.py` | 直接渲染 JSON 为图谱（使用本地 assets 颜色映射） |
| `scripts/build_layered_prototype.py` | 由 export 的 JSON 产出分层 2D HTML |
| `scripts/fix_html_stale.py` / `scripts/fix_html_cdn.py` | 清理旧 HTML 的失效/外部脚本引用 |
