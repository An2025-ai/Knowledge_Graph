...
def build_candidate_rows(
    ...
    *,
    ...
    for cand in candidates:
        ...
        cand_id = f"KC_{context.get('document_id') or 'doc'}_{_stable_hash(*suffix_keys)}"
        if cand_id in seen:
            continue
        seen.add(cand_id)
...
def _stable_hash(*parts: Any) -> str:
    """Build the same deterministic short hash used by the legacy writer.

    Keeping this small value-object helper here means row construction remains
    usable by the local SQLite runtime without importing legacy database code.
    """
